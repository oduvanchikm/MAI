...
int solve_expresion(Node* root)
{
    ...
    int left_node = solve_expresion(root->left);
    int right_node = solve_expresion(root->right);
...
void create_truth_table(FILE* output_file, char* name_of_variables, int count_of_variables, Node* root)
{
    ...
    for (int row = 0; row < num_rows; row++)
    {
        ...
        int result = solve_expresion(root);
...
status_code all_functions(char* filename)
{
    ...
    {
        if (isalpha(postfix_expression[i])/* || postfix_expression[i] == '1' || postfix_expression[i] == '0'*/)
        {
